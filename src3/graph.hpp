...
public : 
    ...
    // グラフファイル読み込み
    void init(const std::string& dir_path, const std::string& hostname, const std::string& hostip);

    // 辺の追加
    void addEdge(const std::vector<std::string>& words, const std::string& hostip);
    ...
    // ノードの持ち主の IP アドレスを入手
    std::string getIP(const uint32_t node_ID);
...
private :
    ...
    std::unordered_map<uint32_t, std::string> vertices_IP_; // 自サーバが保持しているノードの持ち主の IP アドレス {ノード ID : IP アドレス (ノードの持ち主)}
...
inline void Graph::init(const std::string& dir_path, const std::string& hostname, const std::string& hostip) {
...
inline void Graph::addEdge(const std::vector<std::string>& words, const std::string& hostip) { // words : [ノード 1, ノード 2, ノード 2 の IP アドレス]
    my_vertices_.insert(std::stoi(words[0])); // ノード 1 の持ち主は自分
    vertices_IP_[std::stoi(words[0])] = hostip; // ノード 1 の IP アドレスを代入
    vertices_IP_[std::stoi(words[1])] = words[2]; // ノード 2 の IP アドレスを代入
...
inline std::string Graph::getIP(const uint32_t node_ID) {
