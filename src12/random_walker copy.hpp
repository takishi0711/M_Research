...
// 経路長 : 8bit
// RWerID : 4B
// 通信が発生した時の遷移先 index : 4B <- なし？
// 経路情報 : 8B単位の配列
// HostIP, 同一サーバ内の経路長, {頂点, 次数, u->v の index, v->u の index, 頂点, 次数, ...}, HostIP, 同一サーバ内の経路長, ...
// 1 頂点目の index 部分は通信が発生した遷移をbitで管理するスペースに (8B + 8B = 16B)

// RWer のサイズは 1 + 1 + 1 + 1 + 4 + 4 + (4 + 4 + 4 + 4 + 4)*50 = 1012B (送信経路上では 12 + 20*path_length_) <- 前バージョン
// RWer のサイズは 1 (メッセージID, 終了しているかのフラグ, 一歩前で通信が発生したか) + 1 (経路長) + 4 (RWerID) + (8*5) * 50 (大体の経路情報) = 2000B くらい
